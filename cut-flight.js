...
GameEngine=class extends FlightBaseEngine{
 ...
 drawNinja(){const c=this.ctx;this.text('Cut the fruit box with your answer · A D choose · Space / X cut',615,25,16,'#d7e8ed','center');
  for(let i=0;i<this.options.length;i++){if(this.fruitPieces.some(p=>p.index===i))continue;const p=this.cardPos(i);this.fruitFace(i,p.x,p.y);if(i===this.focusCard&&!game.solved){c.save();c.setLineDash([7,5]);this.rounded(p.x-163,p.y-95,326,180,16,null,this.cutAttack?'#ffe2ac':'#a9edda');c.restore();}}
