...
function learning(){setScreen('learning');$('#main').innerHTML=`<section class="page"><div class="page-title"><div><h2>Your learning goals</h2><p>Things Around Me · Grade 7 · Simple English</p></div>${button('Back','back','btn small')}</div><div class="scroll"><div class="grid"><div class="card full"><h3>Read and understand</h3><p>${LEARNING.cp}</p><p class="source">${LEARNING.source}</p></div><div class="card"><h3>Curriculum Mapping</h3>${(()=>{
    const m = db.customMeta || {};
    const subject = m.subject || 'English';
    const phase = m.phase || 'Phase D (Grade VII)';
    const theme = m.theme || 'Things Around Me';
    const element = m.element || 'Reading–Viewing';
    const cp = m.cp || 'Memahami alur informasi, informasi tersurat dan tersirat dari berbagai jenis teks tertulis atau teks multimodal tentang topik sehari-hari atau yang sesuai dengan minat dan meresponnya sesuai konteks.';
    const tp = m.tp || 'Peserta didik mampu memahami dan merespons informasi tersurat dan tersirat dari teks pendek dan teks multimodal sederhana tentang benda, tempat, aktivitas, dan lingkungan sehari-hari melalui konteks petualangan interaktif';
    return `<p style="font-size:15px;line-height:1.5;margin-top:10px;">${esc(subject)} – ${esc(phase)}<br>Theme: ${esc(theme)}<br>Main Element: ${esc(element)}<br>CP:<br>${esc(cp)}<br>TP:<br>${esc(tp)}<br><br></p>`;
  })()}<h3 style="margin-top:20px;">What you will learn</h3>${(db.customMeta?.goals || LEARNING.tp).map((t,i)=>`<div class="tp"><strong>0${i+1}</strong><span>${esc(t)}</span></div>`).join('')}</div><div class="card"><h3>Three steps</h3><p><b>Discover:</b> use words in daily life.<br><br><b>Connect:</b> read clues about places, times and steps.<br><br><b>Solve:</b> use all the clues to choose an answer.</p><h3 style="margin-top:20px">Your score</h3><p>Each correct test answer gives you 10 points. Get 70 or more to open the next area. Game points and lives are separate from your test score.</p></div></div></div></section>`;}
...
function teacherEditor(){
  ...
  <div class="book-tabs">${AREA_META.map((a,i)=>`${button(a.name,'editor-tab',i===editorArea?'btn active':'btn ghost',`data-i="${i}"`)}`).join('')} ${button('Curriculum Settings','editor-meta',editorArea===-1?'btn active':'btn ghost')}</div>
  <div class="scroll">${editorArea===-1 ? `
    <div class="card" style="padding:20px;">
      <h3 style="margin-bottom:15px;color:#a9f1cc;">Edit Curriculum Metadata</h3>
      <label>Subject</label>
      <input type="text" id="meta-subject" value="${esc(db.customMeta?.subject || 'English')}">
      <label>Phase / Grade</label>
      <input type="text" id="meta-phase" value="${esc(db.customMeta?.phase || 'Phase D (Grade VII)')}">
      <label>Theme</label>
      <input type="text" id="meta-theme" value="${esc(db.customMeta?.theme || 'Things Around Me')}">
      <label>Main Element</label>
      <input type="text" id="meta-element" value="${esc(db.customMeta?.element || 'Reading–Viewing')}">
      <label>CP (Capaian Pembelajaran)</label>
      <textarea id="meta-cp" style="width:100%;height:100px;margin-bottom:12px;padding:12px;background:#0b1a2c;color:white;border:1px solid #3c546a;border-radius:8px;">${esc(db.customMeta?.cp || 'Memahami alur informasi, informasi tersurat dan tersirat dari berbagai jenis teks tertulis atau teks multimodal tentang topik sehari-hari atau yang sesuai dengan minat dan meresponnya sesuai konteks.')}</textarea>
      <label>TP (Tujuan Pembelajaran)</label>
      <textarea id="meta-tp" style="width:100%;height:100px;margin-bottom:12px;padding:12px;background:#0b1a2c;color:white;border:1px solid #3c546a;border-radius:8px;">${esc(db.customMeta?.tp || 'Peserta didik mampu memahami dan merespons informasi tersurat dan tersirat dari teks pendek dan teks multimodal sederhana tentang benda, tempat, aktivitas, dan lingkungan sehari-hari melalui konteks petualangan interaktif')}</textarea>
      <label>Learning Goals (Comma separated)</label>
      <textarea id="meta-goals" style="width:100%;height:100px;margin-bottom:12px;padding:12px;background:#0b1a2c;color:white;border:1px solid #3c546a;border-radius:8px;">${esc((db.customMeta?.goals || LEARNING.tp).join(','))}</textarea>
      ${button('Save Curriculum Data', 'editor-save-meta', 'btn primary')}
    </div>
  ` : `<div style="display:flex;flex-direction:column;gap:12px;">
    ${(()=>{TESTS[editorArea].forEach((tq,i)=>tq.id='t'+editorArea+'-'+i); return [...MISSION_BANKS[editorArea], ...TESTS[editorArea]];})().map((q, j) => {
      ...
      return `<div class="card" style="padding:15px;display:flex;justify-content:space-between;align-items:center;">
        <div>
          <b style="font-size:16px;color:#a9f1cc">${esc(activeQ.icon)} ${esc(activeQ.text)}</b>
          <div style="font-size:14px;color:#adc4d8;margin-top:6px;">Ans: ${esc(activeQ.answer)} | Type: ${esc(activeQ.type)} ${custom?'<span style="color:#ffdb94;margin-left:10px;">(Modified)</span>':''}</div>
        </div>
...
function editorExport() {
  const exportObj = { customBanks: db.customBanks, customMeta: db.customMeta }; const safe = JSON.stringify(exportObj, null, 2);
...
function startPlay(){closeModal();setScreen('play');const cp=profile().checkpoints[area];game=cp?{...cp}:{index:0,correct:0,answered:0,assisted:0,points:0};game.questionIds=validMissionSet(cp?.questionIds,area)?cp.questionIds.slice():cp?MISSION_BANKS[area].slice(0,12).map(q=>q.id):drawMissionSet(area);game.resumeOptions=cp?.options;game.hints=3;game.lives=cp?.lives??3;game.combo=0;game.over=!!cp?.over;if(game.over||game.lives<=0){showGameOver();return;}
if(game.index===0&&!game.guideShown){
    game.guideShown=true;
    const guides = [
        {name: 'Kirana', icon: '👩🏽', title: 'School Sprint', msg: 'Run and jump to the correct answer! Use your keyboard or on-screen controls to avoid barricades.'},
        {name: 'Sari', icon: '👱🏽‍♀️', title: 'Household Launch', msg: 'Aim the slingshot at the right word. You can also use Help me shoot if you are stuck!'},
        {name: 'Siti', icon: '🧕🏽', title: 'Neighborhood Rush', msg: 'Tap or use keys to change lanes and pick the correct path. Think fast!'},
        {name: 'Retno', icon: '👧🏽', title: 'Crystal Finale', msg: 'Swipe or press space to slice the correct answer box to find the crystal pieces!'}
    ];
    const g = guides[area];
    modal(`<div class="eyebrow">${g.title} Guide</div><div class="dialogue" style="margin-top:15px;align-items:center;display:flex;gap:15px;"><div class="mascot" style="font-size:54px;line-height:1;">${g.icon}</div><div><h3 style="margin-bottom:6px;">${g.name} says:</h3><p style="font-size:16px;">${g.msg}</p></div></div><div class="row" style="margin-top:20px;">${button('Start Mission','guide-start','btn primary')}</div>`);
} else {
    nextMission();
}}
...
class GameEngine{
 ...
 drawSchool(){const c=this.ctx;this.rounded(0,308,1230,25,0,'#416477');this.text('Jump onto the platform with your answer.',25,32,17,'#b9d6e5');for(let i=0;i<this.options.length;i++){const x=370+i*(1150/this.options.length)-this.cam,y=235-(game.index%3)*8,active=game.solved&&this.target===i;this.rounded(x-110,y,220,18,7,active?'#91edb7':'#477287',active?'#b7ffd0':'#74a4b1');this.rounded(x-140,66,280,75,12,active?'#234f49':'#0b2135','#4a6b81');this.wrap(`${i+1}. ${this.options[i]}`,x,95,260,18);this.object(this.options[i],x,y-39,65);if(active&&!this.sprite(14,x+48,y-35,36))this.text('◇',x+48,y-28,27,'#b1ffc9','center');}for(const box of SCHOOL_OBSTACLES){this.text('🚧',box.x+box.w/2-this.cam,box.y+21,30,'white','center');}const p=this.player;c.globalAlpha=this.cooldown>0?.6:1;if(!this.sprite(12,p.x-this.cam,p.y-28,64))this.text(profile()?.avatar||'A',p.x-this.cam,p.y-5,48,'white','center');c.globalAlpha=1;if(this.joystick){c.beginPath();c.arc(this.joystick.origin.x,this.joystick.origin.y,45,0,Math.PI*2);c.fillStyle='#ffffff22';c.fill();c.strokeStyle='#ffffff44';c.lineWidth=2;c.stroke();c.beginPath();c.arc(this.joystick.current.x,this.joystick.current.y,20,0,Math.PI*2);c.fillStyle='#ffffff66';c.fill();}}
