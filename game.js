...
function newProfile(name,avatar){return {id:Date.now().toString(36)+Math.random().toString(36).slice(2,7),name,avatar,points:0,tests:[null,null,null,null],done:[false,false,false,false],checkpoints:[null,null,null,null],badges:[],history:[],streak:1,lastDate:new Date().toLocaleDateString('en-CA'),egg:[],completed:null};}
...
function nameModal(){selectedAvatar='🧑‍🚀';modal(`<div class="eyebrow">NEW PLAYER</div><h2 style="margin-top:12px">What is your name?</h2><p>We will use this name on your results and certificate.</p><label for="player-name">Player name</label><input id="player-name" type="text" maxlength="32" placeholder="Type your name" autocomplete="off"><label>Choose your picture</label><div class="avatars">${['🧑‍🚀','🧑‍🎓','🤖','🐱'].map((a,i)=>button(a,'avatar',i===0?'active':'',`data-value="${a}" aria-label="Avatar ${i+1}"`)).join('')}</div><div class="row">${button('Start exploring →','create','btn primary')}${button('Cancel','close','btn ghost')}</div>`);}
function createPlayer(){const name=$('#player-name').value.trim();if(!name){toast('Please type your name first.');$('#player-name').focus();return;}const p=newProfile(name,selectedAvatar);db.profiles.push(p);db.active=p.id;save();closeModal();prologStep=0;prolog();}
...
function players(){setScreen('players');$('#main').innerHTML=`<section class="page"><div class="page-title"><div><h2>Choose a player</h2><p>Each player has their own progress on this device.</p></div>${button('+ New player','new-player','btn primary')}</div><div class="scroll"><div class="grid">${db.profiles.map(p=>`<div class="card"><h3>${p.avatar} ${esc(p.name)}</h3><p>${completed(p)}/4 crystals · ${p.points} game points · ${p.streak} days of practice</p><div class="row" style="margin-top:15px">${button('Play as this player','select-player','btn small',`data-id="${p.id}"`)}</div></div>`).join('')||'<p>No players yet. Add your name to start.</p>'}</div></div></section>`;}
...
class GameEngine{
 constructor(canvas,type,options){this.canvas=canvas;this.ctx=canvas.getContext('2d');this.type=type;this.options=options;this.running=false;this.paused=false;this.ready=false;this.target=-1;this.bonus=0;this.keys=new Set();this.t=0;this.player={x:80,y:295,vx:0,vy:0,ground:true};this.cam=0;this.lane=1;this.carX=615;this.gateY=20;this.ball={x:100,y:256,vx:0,vy:0,flying:false};this.drag=null;this.cooldown=0;this.particles=[];this.trail=[];this.bg=new Image();this.bg.src='assets/world.webp';this.atlas=new Image();this.atlas.src='assets/sprites.webp';this.abort=new AbortController();const signal=this.abort.signal;canvas.addEventListener('pointerdown',e=>this.pointerDown(e),{signal});canvas.addEventListener('pointermove',e=>this.pointerMove(e),{signal});canvas.addEventListener('pointerup',e=>this.pointerUp(e),{signal});canvas.addEventListener('pointercancel',()=>{this.drag=null;this.swiping=false;},{signal});}
 ...
 pointerDown(e){if(this.paused)return;this.canvas.setPointerCapture?.(e.pointerId);const p=this.point(e);if(this.type===1&&this.ready&&!this.ball.flying&&Math.hypot(p.x-this.ball.x,p.y-this.ball.y)<95)this.drag=p;if(this.type===2){const w=1230/this.options.length; this.lane=Math.max(0,Math.min(this.options.length-1,Math.floor(p.x/w)));}if(this.type===3){this.swiping=true;this.hitCards(p);}if(p.x>1180&&p.y<45)this.findEgg();}
 pointerMove(e){if(this.paused)return;const p=this.point(e);if(this.drag){const dx=p.x-100,dy=p.y-256,d=Math.hypot(dx,dy),s=Math.min(1,125/(d||1));this.drag={x:100+dx*s,y:256+dy*s};}if(this.swiping){this.trail.push({...p,life:.35});this.hitCards(p);}}
 pointerUp(e){if(this.drag&&this.ready){this.ball.x=100;this.ball.y=256;this.ball.vx=(100-this.drag.x)*7;this.ball.vy=(256-this.drag.y)*7;this.ball.flying=true;this.drag=null;tone(330,.12);}this.swiping=false;}
