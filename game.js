...
if(game.index===0&&!game.guideShown){
    ...
    const g = guides[area];
    setScreen('guide');
    $('#main').innerHTML=`<section class="prolog"><div class="dialogue"><div class="mascot" style="font-size:76px;line-height:1;">${g.icon}</div><div style="flex:1;"><div class="eyebrow">${g.title} Guide</div><h2>${g.name} says:</h2><p>${g.msg}</p></div>${button('Start Mission →','guide-start','btn primary')}</div></section>`;
} else {
    nextMission();
}}
