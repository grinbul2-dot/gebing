...
GameEngine=class extends PhysicsEngine{
 ...
 hitSegment(a,b){if(game.solved||game.feedback)return;let first=null;for(let i=0;i<3;i++){const p=this.cardPos(i);const t=this.segmentRect(a,b,{x:p.x-166,y:p.y-78,w:332,h:156});if(t!==null&&(!first||t<first.t))first={i,t};}if(first)answerByMove(first.i);}
 ...
 drawRunner(x,y,size){const c=this.ctx,sw=this.atlas.width/4,sh=this.atlas.height/4,srcX=0,srcY=3*sh,p=this.player,moving=p.ground&&Math.abs(p.vx)>1,phase=this.runPhase,bob=moving?Math.abs(Math.sin(phase))*2:Math.sin(this.t*2)*.5,crouch=this.keys.has('ArrowDown')&&p.ground,squash=this.landSquash>0?Math.sin(this.landSquash/.2*Math.PI)*.14:0;
  ...
  const scale=0.95;
  c.drawImage(this.atlas,srcX,srcY,sw,sh,-size*scale/2,-size*scale*0.8,size*scale,size*scale*(sh/sw));c.restore();
 }
 ...
 drawRace(){const c=this.ctx,top=45,bottom=333,road=(y)=>135+(y-top)/(bottom-top)*480;
  ...
  for(let i=0;i<12;i++){const z=((i/12+this.roadTravel*.0015)%1),y=top+z*z*(bottom-top),half=road(y);for(const side of[-1,1]){const x=615+side*half;c.strokeStyle=i%2?'#d8e2b7':'#69bc92';c.lineWidth=2+z*6;c.beginPath();c.moveTo(x,y);c.lineTo(x+side*(8+z*10),y+5+z*8);c.stroke();}for(const lane of[-1,1]){const x=615+lane*half/3;c.strokeStyle='#d5e4dc';c.lineWidth=1+z*3;c.beginPath();c.moveTo(x,y);c.lineTo(x+lane*z*5,y+4+z*9);c.stroke();}}
  for(let i=0;i<3;i++){const y=this.gateY+8,z=Math.max(0,Math.min(1,(y-20)/250)),w=170+z*177,x=615+(i-1)*(195+z*215),active=game.solved&&this.target===i;this.rounded(x-w/2,y,w,84,10,active?'#2b624c':'#10283d',active?'#baffd0':'#7297aa');this.wrap(`${i+1}. ${this.options[i]}`,x,y+29,w-16,Math.round(13+z*5));this.text(active?'↓ YOUR TARGET':'GATE',x,y+74,12,active?'#caffcf':'#a7c6d7','center');}
 ...
 drawNinja(){const c=this.ctx;this.text('Swipe across the card · A D choose · Space / X cut',615,29,18,'#d1d3ef','center');
  for(let i=0;i<3;i++){if(this.cutPieces.some(p=>p.index===i))continue;const p=this.cardPos(i);this.cardFace(i,p.x,p.y);if(i===this.focusCard&&!game.solved){c.save();c.setLineDash([8,6]);this.rounded(p.x-171,p.y-83,342,166,24,null,'#d1fbea');c.restore();}}
